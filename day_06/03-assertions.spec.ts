...
test(`Learn to  handle dropdown`,async ({page}) => {
  ...
  for (let index = 0; index <  dropdownCount ; index++) { // Print all the 13 values
    ...
    const statusLoactor = page.locator(`#viewLead_statusId_sp`); // locator value of "Assigned"

    //Locator Assertion

     await expect(statusLoactor).toBeVisible()

    //Genric Location

    const statusText = await statusLoactor.innerText();

    console.log(`The status is ${statusText}`);

    await expect(statusLoactor).toContainText("Assigned")
